...
- **Big Pickle Proxy**: `bigpickle/` routes through the local proxy at `http://localhost:8000/v1` (port 8000). Start the proxy first: `cd bigpickle-proxy && python proxy.py --port 8000`. Free models: `big-pickle` (DeepSeek V3), `deepseek-v4-flash-free`, `nemotron-3-super-free`, `mimo-v2.5-free`. No API key needed — the proxy handles UUID auth internally. **Important**: `big-pickle` is a reasoning model — its thinking tokens count against `max_tokens`, so chess-llm auto-bumps to 4096. For best reliability, use `--tiny` mode (compact prompt = less reasoning needed). `--reasoning` has no effect through this provider (the proxy strips `reasoning_effort`).
